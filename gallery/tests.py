...
class VideoGetListAddTestCase(APITestCase):
    def setUp(self):
        # create a new user by making a post request to register endpoint
        self.user=self.client.post('/register/',data={'email':'mario@testmail.com','password':'i-keep-jumping', 'first_name':'mario', 'last_name':'icardi', 'phone': '+2348122633167', 'address': "here"})
        # obtain a json web token for the newly created user
    ...
    def test_video_add_non_admin(self):
        """
        Ensure video cannot be added by non-admin
        """
        self.api_authentication()
        data={"url":"https://www.youtube.com/watch?v=54odvrcEpUc&ab_channel=TransparentFxAcademy"}
        response=self.client.post('/videos/add/',data)
        self.assertEqual(response.status_code,status.HTTP_403_FORBIDDEN)

    def test_video_add_admin(self):
        """
        Ensure video can be added by admin
        """
        self.client.force_authenticate(user=self.admin)
        data={"url":"https://www.youtube.com/watch?v=54odvrcEpUc&ab_channel=TransparentFxAcademy"}
        response=self.client.post('/videos/add/',data)
